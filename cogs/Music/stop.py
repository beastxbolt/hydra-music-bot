...
class stop(commands.Cog):
    ...
    async def stop(self, ctx):
        ...
        async def stopFunc():
            ...
            embed = discord.Embed(title="Stopped!",colour=0x0000ff)
        ...
        try:
            if ctx.author.voice is not None:
                        ...
                        if ctx.voice_client.is_playing() or ctx.voice_client.is_paused():
                            if self.bot.voice_client_attributes[str(ctx.guild.id)]['block_skip_and_stop'] is False:
                                if self.bot.voice_client_attributes[str(ctx.guild.id)]['radio'] is False:
                                    ...
                                    embed = discord.Embed(title="Cannot Stop Music While Radio Is Playing!", description="Use `radio stop` Command To Stop The Radio.", colour=0xff0000)
                                    await ctx.send(embed=embed)
                                    return
                            else:
                                embed3 = discord.Embed(title="Cannot Stop While Adding Playlist In Queue!",colour=0xff0000)
                ...
                embed7 = discord.Embed(title="You Must Be In A Voice Channel To Use This Command!",colour=0xff0000)
